...
class Run():
    ...
    def get_dataloader_temporal(self):
        ...
        dataset_train = BSVFNDDataset('vid_time3_train.txt',token,self.dataset)
        dataset_val = BSVFNDDataset('vid_time3_val.txt',token,self.dataset)
        dataset_test = BSVFNDDataset('vid_time3_test.txt',token,self.dataset)
    ...
    def get_model(self):
        self.model = BSVFNDModel(fea_dim=128,dropout=self.dropout,dataset=self.dataset)
        return self.model
